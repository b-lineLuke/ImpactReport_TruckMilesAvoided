function runClientsReport(client) {
  // This function is the same as impactReport() except it takes a client as an argument and it returns an array as opposed to
  // setting a google sheet.
  ...
  var ss2022ImpactReporting = SpreadsheetApp.openByUrl("https://docs.google.com/spreadsheets/d/1zfP-411KBt20GKKDLg_Ta72UJHxGspmJymRdThSLi3E/edit#gid=866382810");
  ...
  // FOR i < listOfDays.length ...
  for (var i = 0; i < listOfDays.length; i++) {
    // NOTE: This for loop iterates through every day that the client had items delivere to an account.
